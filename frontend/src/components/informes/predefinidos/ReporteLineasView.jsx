...
const SectionReporte = ({ linea, totalMetasGlobal, isInternalReport }) => {
    const { counts, metas, nombre } = linea;
    const totalMetas = metas.length;

    // Colores oficiales
    // const COLORS = ['#000000', '#D32F2F', '#F57C00', '#FBC02D', '#388E3C'];
    ...
    return (
        <div className="mb-12 break-inside-avoid border-b pb-8 last:border-0">
            <h2 className="text-xl font-bold mb-4 uppercase text-blue-800 border-l-4 border-blue-800 pl-2">{nombre}</h2>
...
const ReporteLineasView = ({ data, isInternalReport, onClose }) => {
    if (!data) return null;

    const { plan, lineas } = data;
    ...
    return (
        <div className="fixed inset-0 z-50 bg-white overflow-auto">
            ...
            <div className="sticky top-0 z-10 bg-gray-100 p-4 border-b flex justify-between items-center print:hidden">
                <h2 className="text-xl font-bold text-gray-800">Vista Previa: Reporte por Líneas</h2>
            ...
            <div className="print:w-full max-w-[210mm] mx-auto p-[20mm] bg-white print:p-0 print:m-0">
                ...
                <div className="text-center mb-8 border-b pb-4">
                    ...
                    <p className="text-sm font-semibold mt-2">REPORTE CONSOLIDADO POR EJE ESTRATÉGICO / LÍNEA</p>
                </div>

                {/* Iteración de Líneas */}
                {lineas.map((linea, idx) => (
                    <SectionReporte key={idx} linea={linea} isInternalReport={isInternalReport} />
                ))}
...
export default ReporteLineasView;
