...
const ReporteRankingSecretariasView = ({ data, onClose }) => {
    ...
    const getRowStyle = (p) => {
        // Colores solicitados (mismo esquema que reporte 4):
    ...
    return (
        <div className="fixed inset-0 z-50 bg-white overflow-auto animate-in fade-in duration-300">
            ...
            <div className="sticky top-0 z-10 bg-gray-100 p-4 border-b flex justify-between items-center print:hidden">
                <div>
                    <h2 className="text-xl font-bold text-gray-800">6. Ranking por Dependencia</h2>
            ...
            <div className="p-8 max-w-5xl mx-auto bg-white print:p-0 print:max-w-none">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-2xl font-bold uppercase mb-2">RANKING DE DEPENDENCIAS POR AVANCE</h1>
                    ...
                    <p className="text-xs text-gray-400 mt-1 print:hidden">
                        Total de Secretarías listadas: {ranking.length}
                    </p>
                </div>
                    ...
                    <table className="w-full border-collapse border border-gray-300 shadow-sm print:shadow-none print:border-gray-800">
                        <thead className="bg-gray-100 text-gray-700 print:bg-gray-200">
                            <tr>
                                ...
                                <th className="border p-3 text-left">Secretaría / Dependencia</th>
                                <th className="border p-3 text-center w-24">Metas Asignadas</th>
                                <th className="border p-3 text-center w-32">% Avance Relativo</th>
                                <th className="border p-3 text-center w-24">Progreso</th>
                            </tr>
                        </thead>
                        <tbody>
                            {ranking.map((item, index) => (
                                <tr key={item.secretaria_id} className={`border-b transition-colors ${getRowStyle(item.avance_porcentaje)}`}>
                            ...
                            {ranking.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="border p-6 text-center text-gray-500 bg-white">
...
export default ReporteRankingSecretariasView;
