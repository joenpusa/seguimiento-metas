...
const SectionReporte = ({ secretaria, totalMetasGlobal, isInternalReport }) => {
    const { counts, metas, nombre } = secretaria;
    ...
    return (
        <div className="mb-12 break-inside-avoid border-b pb-8 last:border-0">
            <h2 className="text-xl font-bold mb-4 uppercase text-teal-800 border-l-4 border-teal-800 pl-2">{nombre}</h2>
...
const ReporteSecretariasView = ({ data, isInternalReport, onClose }) => {
    if (!data) return null;

    const { plan, secretarias } = data;
    ...
    return (
        <div className="fixed inset-0 z-50 bg-white overflow-auto">
            ...
            <div className="sticky top-0 z-10 bg-gray-100 p-4 border-b flex justify-between items-center print:hidden">
                <h2 className="text-xl font-bold text-gray-800">Vista Previa: Reporte por Secretaría</h2>
            ...
            <div className="print:w-full max-w-[210mm] mx-auto p-[20mm] bg-white print:p-0 print:m-0">
                ...
                <div className="text-center mb-8 border-b pb-4">
                    ...
                    <p className="text-sm font-semibold mt-2">REPORTE CONSOLIDADO POR SECRETARÍA / DEPENDENCIA</p>
                </div>

                {/* Iteración de Secretarias */}
                {secretarias.map((sec, idx) => (
                    <SectionReporte key={idx} secretaria={sec} isInternalReport={isInternalReport} />
                ))}
...
export default ReporteSecretariasView;
